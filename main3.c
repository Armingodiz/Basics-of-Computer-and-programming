...
int save_game(FILE *save1, FILE *save2, struct cell *header,int number_of_cells,block map[n+1][n+1]);
...
int main()
{
    ...
    case 3:
        ...
        creat_map(n,map);
        print_map(visual_map,n);
        save_game(save1,save2,header1,number_of_cells,map);
        while(1)
        {
            int z=show_list2();
...
void creat_map(int n,block a[n+1][n+1])
{
    ...
    for(int i=0; i<n; i++)
    {
        for(int j=0; j<n; j++)
        {
            ...
            {
                visual_map[x_center][y_center-1]='1';
                visual_map[x_center][y_center]='0';
                visual_map[x_center][y_center+1]='0';
            }*/
        }
    }
}
...
int save_game(FILE *save1, FILE *save2, struct cell *header,int number_of_cells,block map[n+1][n+1])
{
    ...
    for(int j=0; j<n+1; j++)
    {
            fwrite(map[j], sizeof(block), n+1, save2);
    }
    fclose(save1);
    fclose(save2);
    ...
    {
        printf("1");
        fread(&current2, sizeof(struct cell), 1, save1);
        printf("1");
        printf("%d %d %d\n",current2->x,current2->y,current2->energy);
        printf("1");
